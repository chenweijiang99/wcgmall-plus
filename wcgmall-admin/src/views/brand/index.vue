  ...
  <div class="app-container">
    ...
    <div class="search-wrapper">
      <el-form :model="queryParams" ref="queryFormRef" inline>
        ...
        <el-form-item>
          <el-button
            v-permission="['sys:brand']"
            type="primary"
            icon="Search"
            @click="handleQuery"
            >搜索</el-button
          >
          <el-button v-permission="['sys:brand']" icon="Refresh" @click="resetQuery"
            >重置</el-button
          >
        </el-form-item>
      </el-form>
...
const getList = async () => {
  loading.value = true;
  try {
    const { data } = await listProductBrandApi(queryParams);
...
const handleUpdate = (row: any) => {
  reset();
  detailProductBrandApi(row.id).then((response) => {
    ...
    if (valid) {
      try {
        ...
        if (form.id !== undefined) {
          await updateProductBrandApi(form);
          ElMessage.success("修改成功");
        } else {
          await addProductBrandApi(form);
...
const handleBatchDelete = async () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      await deleteProductBrandApi(selectedIds.value);
...
const handleDelete = async (row: any) => {
  ElMessageBox.confirm('是否确认删除编号为"' + row.id + '"的数据项?', "警告", {
    ...
    try {
      await deleteProductBrandApi(row.id);
      ElMessage.success("删除成功");
      getList();
    } catch (error) {
      ElMessage.error("删除失败");
