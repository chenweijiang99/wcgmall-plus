  ...
  <div class="app-container">
    ...
    <div class="search-wrapper">
      <el-form :model="queryParams" ref="queryFormRef" inline>
        ...
        <el-form-item>
          <el-button
            v-permission="['sys:category']"
            type="primary"
            icon="Search"
            @click="handleQuery"
            >搜索</el-button
          >
          <el-button v-permission="['sys:category']" icon="Refresh" @click="resetQuery"
            >重置</el-button
          >
        </el-form-item>
      </el-form>
...
const getList = async () => {
  loading.value = true;
  try {
    const { data } = await listProductCategoryApi(queryParams);
...
const handleUpdate = (row: any) => {
  reset();
  detailProductCategoryApi(row.id).then((response) => {
    ...
    if (valid) {
      try {
        ...
        if (form.id !== undefined) {
          await updateProductCategoryApi(form);
          ElMessage.success("修改成功");
        } else {
          await addProductCategoryApi(form);
...
const handleBatchDelete = async () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      await deleteProductCategoryApi(selectedIds.value);
