...
import {
  listIndexSliderApi,
  detailIndexSliderApi,
  deleteIndexSliderApi,
  addIndexSliderApi,
  updateIndexSliderApi,
} from "@/api/slider/indexSlider";
...
const handleUpdate = (row: any) => {
  reset();
  detailIndexSliderApi(row.id).then((response) => {
    ...
    if (valid) {
      try {
        if (form.id !== undefined) {
          await updateIndexSliderApi(form);
          ElMessage.success("修改成功");
        } else {
          await addIndexSliderApi(form);
...
const handleBatchDelete = async () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      await deleteIndexSliderApi(selectedIds.value);
...
const handleDelete = async (row: any) => {
  ElMessageBox.confirm('是否确认删除编号为"' + row.id + '"的数据项?', "警告", {
    ...
    try {
      await deleteIndexSliderApi(row.id);
      ElMessage.success("删除成功");
      getList();
    } catch (error) {
      ElMessage.error("删除失败");
